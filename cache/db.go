	...
	for rr := range r {
		for cc := range c {
			go func(one string, two string) {
				wg.Add(1)
				var d int
				if err := CDB.QueryRow("select 1 from information_schema.tables where table_name=$1", one+"_"+two).Scan(&d); err == pgx.ErrNoRows {
					_, err := CDB.Exec(`create table "` + one + "_" + two + `"(created_at timestamp with time zone, updated_at timestamp with time zone, key text, response jsonb)`)
					if err != nil {
						log.Fatal(err)
					}
				}
